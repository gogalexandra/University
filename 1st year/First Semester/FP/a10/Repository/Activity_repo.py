...
class ActivityRepository:
    ...
    def __init__(self, activity_list):
        self._activities = DataStructure()
    ...
    def delete_activity(self, idd):
        ...
        activity_object = self.does_id_exist(idd)
        if activity_object is None:
            raise ActivityRepositoryException("Activity does not exist!!!!")

        a = 0
        while a <= len(self._activities) - 1:
            if self.activities[a].activity_id == idd:
                del self._activities[a]
                self.save()
            else:
                a += 1
    ...
    def update_activity(self, activity: Activity):
        ...
        activity_object = self.does_id_exist(activity.activity_id)
        if activity_object is None:
            raise ActivityRepositoryException("Person does not exist!!!!")

        a = 0
        while a <= len(self._activities) - 1:
            if self.activities[a].activity_id == activity.activity_id:
                self.activities[a] = activity
                self.save()
            a += 1
