...
Table::Table(std::vector<Dog>& designatedVector, QObject* parent) : inputVector(designatedVector) {}
...
QVariant Table::data(const QModelIndex& index, int role) const {
    ...
    /*if (role == Qt::DisplayRole) {
        if (column == 0) {
            std::string breed = this->inputVector[row].getBreed();
            return QString::fromStdString(breed);
        }
        else if (column == 1) {
