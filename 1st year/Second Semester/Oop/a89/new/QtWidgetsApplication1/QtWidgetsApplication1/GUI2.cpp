...
void GUI::start_admin()
{
	...
	button_ly->addWidget(undo);
	button_ly->addWidget(redo);
	button_ly->addWidget(back);
...
void GUI::add(QVBoxLayout* start_ly)
{
	...
	QObject::connect(done, &QPushButton::clicked, this, [=]() {
		...
		try {
			...
			Dog d(breed, name, age, photp);
			AddOperation* newOperation = new AddOperation(d, this->s.getRepo2());
			this->e->saveOperation(newOperation);
		}
...
void GUI::del()
{
	...
	if (index != -1) {
		...
		DeleteOperation* newOperation = new DeleteOperation(d[index], this->s.getRepo2());
		this->e->saveOperation(newOperation);
		this->populateList();
	}
