...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        ...
        for i in v:
            if 0 <= self.x + i[0] <= 19 and 0 <= self.y + i[1] <= 19 and\
                    detectedMap.surface[self.x + i[0]][self.y + i[1]] == 0 and \
                    [self.x + i[0], self.y + i[1]] not in self.visited:
                self.stack.append([self.x + i[0], self.y + i[1]])
        new_pos = self.stack.pop(-1)
        while new_pos in self.visited and len(self.stack) != 0:
            new_pos = self.stack.pop(-1)
