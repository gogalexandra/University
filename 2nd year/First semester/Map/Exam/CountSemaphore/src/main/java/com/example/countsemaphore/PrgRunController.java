...
public class PrgRunController implements Initializable {
    ...
    @FXML
    private TableView<HashMap.Entry<Integer, Pair<Integer, ArrayList<Integer>>>> semaphoreTable;
    
    @FXML
    private TableColumn<HashMap.Entry<Integer,Pair<Integer, ArrayList<Integer>>>, String> semaphoreTableAddress;
    
    @FXML
    private TableColumn<HashMap.Entry<Integer,Pair<Integer, ArrayList<Integer>>>, String> semaphoreTableValue;
    
    @FXML
    private TableColumn<HashMap.Entry<Integer,Pair<Integer, ArrayList<Integer>>>, String> semaphoreTableArray;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        this.semaphoreTableAddress.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getKey().toString()));
        this.semaphoreTableValue.setCellValueFactory(cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getValue().getKey())));
        this.semaphoreTableArray.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().getValue().toString()));
    ...
    private void populateSemaphoreTable(PrgState currentProgramState) {
        MyIDictionary<Integer, Pair<Integer, ArrayList<Integer>>> semaphoreTable = currentProgramState.getSemaphore().getSemaphore();
        List<Map.Entry<Integer, Pair<Integer, ArrayList<Integer>>>> semaphoreList = new ArrayList<>();

        for (Map.Entry<Integer, Pair<Integer, ArrayList<Integer>>> entry : semaphoreTable.getAll().entrySet())
            semaphoreList.add(entry);
        this.semaphoreTable.setItems(FXCollections.observableList(semaphoreList));
