...
public class PrgRunController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        lockTableAddress.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getKey().toString()));
        lockTableValue.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getValue().toString()));
    ...
    private void populateLockTable(PrgState currentProgramState) {
        MyIDictionary<Integer, Integer> LockTable = currentProgramState.getLockTable().getLock();
        List<Map.Entry<Integer, Integer>> lockList = new ArrayList<>();

        for (Map.Entry<Integer, Integer> entry : LockTable.getAll().entrySet())
            lockList.add(entry);
